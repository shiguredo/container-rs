...
| `with_mapped_port(self, host_port, container_port)` | あり | 対応 | 対応 | `publishedPorts` に反映。`host_port = 0` (Docker のランダム割当の慣用) は Apple container にランダム割当が無いため、expose 経路と同じく空きホストポートを事前割当する。割当は bind(0) → 即 release のため、起動までの間にポートを奪われると start が失敗し得る (自動再試行なし) / Docker: 0 をそのまま送り Docker Engine のランダム割当に任せる |
