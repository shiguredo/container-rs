...
//!
//! テスト本体は機能単位のインライン `mod` (lifecycle / interaction / logs / copy / readiness)
//! に分割している。共通ヘルパーと `use super::*` 経由で供給する import はルートに置く。
...
mod lifecycle {
    //! コンテナの起動・停止・削除と Runtime / Drop の相互作用の検証。
    //!
    //! 起動後コンテナの状態と操作 (exec / ports / exit_code) は interaction モジュールにある。
...
mod interaction {
    //! 起動後コンテナとの相互作用 (exec / ports / exit_code) の検証。
...
mod logs {
    //! ログストリーム (LogConsumer / follow リーダー / 待機) の検証。
...
mod copy {
    //! コンテナへのファイル投入 (`with_copy_to`) と回収 (`copy_file_from`) の検証。
...
mod readiness {
    //! 起動待機 (WaitFor / healthcheck / exit) と起動設定の検証。
