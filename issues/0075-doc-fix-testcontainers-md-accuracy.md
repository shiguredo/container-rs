...
- **14 章 (Mount) の Docker 列が実装と矛盾**: `target()` (543 行)「Volume/Tmpfs は create 時に明示エラー」、`with_size_bytes` / `with_size` / `with_mode` / `tmpfs_options` (544-547 行)「未反映」、`MountType::Volume` / `Tmpfs` (554-555 行)「明示エラー」、`MountTmpfsOptions` (560-561 行)「未反映」はすべて誤り。`src/core/client/docker_client.rs` の `CreateContainerBody::from_config` は Volume / Tmpfs を `HostConfig.Mounts` に、`SizeBytes` / `Mode` を `TmpfsOptions` に反映済み (単体テストあり)。537 / 539 行の「対応 (HostConfig.Mounts に反映)」とも自書矛盾
...
- **「本家との型不整合の注意点」(829-837 行) に未記載の型差分が 5 件** (本家 0.27.3 ソースで確認済み):
...
- **サマリ件数 (72-80 行) が機械集計と不一致**: 記載の 289 / 18 / 4 / 59 / 23 / 5 / 393 に対し、機械集計では 288 / 18 / 4 / 65 / 23 / 5 / 403 になる (0073 修正後の現状を再集計する)
...
- 6.1 節に `pause` 行が追加され、サマリ件数が機械集計と一致する

## 解決方法

- 7 章 235-236 行の `pause` / `unpause` 行を削除し、6.1 節 (191 行付近) に `async fn pause` 行を追加する (本家: あり / Apple: なし / Docker: 対応。備考は「`ContainerAsync` のみ。sync `Container` には無い」)
- 14 章 543 / 544-547 / 554-555 / 560-561 行の判定を「対応」に修正し、「明示エラー」「未反映」の記述を除去する
- 10.3 の表に `with_request_timeout` の行を追加し、CHANGES.md develop に [ADD] として追記する
- 「本家との型不整合の注意点」に 5 件を追記する (表の形式に揃える)
- サマリ件数を再集計して 72-80 行の数値を更新する
