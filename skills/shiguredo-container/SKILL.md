...
| `exec(ExecCommand)` | 対応 (stdout / stderr / env 付き) | exit code のみ (stdout / stderr は空) |
...
- 既定 (`TESTCONTAINERS_COMMAND` 未設定または `remove`) では `ContainerAsync` / `Container` の Drop 時にコンテナを削除する。削除は専用スレッド上のブロッキング API で行い、ユーザーの tokio Runtime 終了に依存しない
