...
| `with_open_stdin`, `with_hostname` | 対応 | start 時に明示エラー |
...
- **Linux の残ギャップ**: `with_platform` / `with_network` / `with_hostname` / `with_host` / `with_open_stdin` / `with_ssh`、Volume / Tmpfs mount が未対応。詳細は `docs/TESTCONTAINERS.md` 参照
